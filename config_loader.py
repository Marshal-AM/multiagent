    ...
    def get_preprocessor_url(id: str) -> str:
        """
        Get preprocessor URL for a client by ObjectId.
        Priority: Environment variable > MongoDB config > Default
        
        Args:
            id: MongoDB ObjectId as string
            
        Returns:
            Preprocessor URL from environment variable or client config
        """
        # First priority: Environment variable
        env_url = os.getenv("PREPROCESSOR_URL")
        if env_url:
            logger.info(f"📡 Preprocessor URL for client {id}: {env_url} (from ENV)")
            return env_url
        
        # Second priority: MongoDB config
        config = get_client_config(id)
        config_url = config.get('preprocessor', {}).get('url')
        if config_url:
            logger.info(f"📡 Preprocessor URL for client {id}: {config_url} (from MongoDB)")
            return config_url
        
        # Final fallback: Default
        default_url = "http://localhost:8080"
        logger.info(f"📡 Preprocessor URL for client {id}: {default_url} (default)")
        return default_url


    def get_postprocessor_url(id: str) -> str:
        """
        Get postprocessor URL for a client by ObjectId.
        Priority: Environment variable > MongoDB config > Default
        ...
        Returns:
            Postprocessor URL from environment variable or client config
        """
        # First priority: Environment variable
        env_url = os.getenv("POSTPROCESSOR_URL")
        if env_url:
            logger.info(f"📡 Postprocessor URL for client {id}: {env_url} (from ENV)")
            return env_url
        
        # Second priority: MongoDB config
        config = get_client_config(id)
        config_url = config.get('postprocessor', {}).get('url')
        if config_url:
            logger.info(f"📡 Postprocessor URL for client {id}: {config_url} (from MongoDB)")
            return config_url
        
        # Final fallback: Default
        default_url = "http://localhost:8003"
        logger.info(f"📡 Postprocessor URL for client {id}: {default_url} (default)")
        return default_url
